...
/*   Updated: 2025/04/19 13:57:36 by hajmoham         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	check_args(int ac, char **av)
{
	...
	if (ac < 5 || ac > 6)
		return (printf("Error: Need 4 or 5 args\n"), 1);
	i = 1;
	while (i < ac)
	{
		j = 0;
		...
		while (av[i][j])
		{
			if (av[i][j] < '0' || av[i][j] > '9')
				return (printf("Error: Non-numeric arg\n"), 1);
			j++;
		}
		if (atoi_ll(av[i]) > INT_MAX || atoi_ll(av[i]) == 0)
			return (printf("Error: Arg too big or zero\n"), 1);
		i++;
	}
	return (0);
}
...
int	parse(t_data *data, int ac, char **av)
{
	...
	if (data->num_philo == 0 || data->time_die == 0 || data->time_eat == 0
		...
		return (printf("Error: Zero value\n"), 1);
	return (0);
}
